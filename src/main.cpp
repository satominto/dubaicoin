...
#if defined(NDEBUG)
# error "Dubaicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7978ac5d27a9532d4b4c602053d3d8f402fae94fe672194693cef392bfcc9ff3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dubaicoin: starting difficulty is 1 / 2^12
...
{
    // Dubaicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dubaicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Dubaicoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Dubaicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dubaicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dubaicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7978ac5d27a9532d4b4c602053d3d8f402fae94fe672194693cef392bfcc9ff3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is my Dubai Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xf62f6bafb59a3ac7807f74c4602f3fe98dbaa439b574347d2232fcc5c0ce27ad"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Dubaicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DubaicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DubaicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DubaicoinMiner(CWallet *pwallet)
{
    printf("DubaicoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running DubaicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DubaicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DubaicoinMiner, pwallet));
}
